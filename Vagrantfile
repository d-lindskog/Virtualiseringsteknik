...
vms = [
  ...
  # Vault flyttas hit till backend-net för högre säkerhet
...
Vagrant.configure("2") do |config|
  ...
  vms.each do |machine|
    config.vm.define machine[:name] do |node|
      ...
      # Primärt nätverkskort [cite: 31, 35, 42]
      node.vm.network "private_network", 
        ip: machine[:ip], 
        virtualbox__intnet: machine[:net]

      # TRIPLE-HOMED FIREWALL [cite: 5, 19]
      if machine[:name] == "firewall"
        # NIC 3: Mot Service-net (DMZ) [cite: 39, 41]
        node.vm.network "private_network", 
          ip: "#{SERVICE_SUB}1", 
          virtualbox__intnet: "service-net"
          
        # NIC 4: Mot Backend-net (Säker zon för Vault/DB) [cite: 44, 46]
        node.vm.network "private_network", 
          ip: "#{BACKEND_SUB}1", 
...
# Provisionering: Ansible-control förbereder nyckeln
      ...
      # Provisionering: Alla maskiner hämtar nyckeln för lösenordsfri inloggning
      node.vm.provision "shell", run: "always", inline: <<-SHELL
        # Vänta på att nyckeln skapas (max 60 sekunder)
        for i in {1..30}; do
